...
5. 추가 기능 계획: 선택한 모드의 `rw-plan-*.prompt.md`
...
- 공통 동작:
  ...
  - `RW_ENV_UNSUPPORTED`와 `MANUAL_FALLBACK_REQUIRED`를 함께 출력한다.
  - 이 환경에서는 자율 루프가 지원되지 않음을 명시한다.
  - 필요한 경우에만 수동 체크리스트를 참고한다.
...
- 운영 검증은 코어 루프를 직접 실행한다:
  ...
  3. `rw-feature`
  4. `rw-plan-lite` (또는 `rw-plan-strict`)
