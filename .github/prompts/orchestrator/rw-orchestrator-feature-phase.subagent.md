...
- Summary resolution (user-first):
  ...
  - Treat summary as unresolved when it is empty or placeholder-level intent (for example: `기능 추가`, `개선`, `업데이트`, `new feature`, `improve`).
...
- Need-gate (HITL priority):
  ...
  - If `NON_INTERACTIVE_MODE=false`, run one mandatory clarification round with up to two focused questions:
